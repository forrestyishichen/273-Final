...
class Coin():
    ...
    def transfer(self, from_wallet, to_wallet, amount):
        ...
        if to_wallet is None:
            return False
        if amount > self.wallets[from_wallet]:
            return False

        try:
            self.wallets[from_wallet] = self.wallets[from_wallet] - amount
            self.wallets[to_wallet] = self.wallets[to_wallet] + amount
        except:
            return False
    ...
    def compute_merkle_root(self, children):
        ...
        for index in range(0,len(children),2):
            ...
            if index+1 != len(children):
                current_right = children[index+1]
            else:
                current_right = ''
        ...
        if len(children) != 1:
            self.compute_merkle_root(temp_transaction)
